...
int getRandomSpeed(string phase) {
    int randomSpeed;

    if (phase == "Holding") {
        randomSpeed = rand() % 251 + 400;
    } 

	else if (phase == "Approach") {
        randomSpeed = rand() % 251 + 240;
    } 

	else if (phase == "Landing") {
        randomSpeed = rand() % 241 + 30;
    } 

	else if (phase == "Taxi") {
        randomSpeed = rand() % 55 + 15;
    } 
        ...
        randomSpeed = rand() % 100 + 200;
    } 

	else if (phase == "Climb") {
        randomSpeed = rand() % 344 + 250;
    } 

	else if (phase == "Departure") {
        randomSpeed = rand() % 711 + 800;
    } 
...
void* handleFlights2(void* arg) {
    ...
    // Lock the runway at the start
    pthread_mutex_lock(mutex);
    if (!runway->isOccupied()) {
        runway->setOccupied(true);
        sf::Clock movementClockA, movementClockB, movementClockC;

		// RUNWAY A case (Arrival flights)
		if (runway->getRunwayID() == "RWY-A") {
			int xlim = 1200;
			float xSpeed = getRandomSpeed(f->getCurrentPhase()) / 100.0f;
			float ySpeed = 2.0f;
			int xpos = f->getSprite().getPosition().x;
			int ypos = f->getSprite().getPosition().y;
			float flightAngle = 90; // Get the angle

			// Holding
			f->setCurrentPhase("Holding");
			cout << f->getCurrentPhase() << " " << f->getID() << endl;
			while(movementClockA.getElapsedTime().asSeconds() < 0.5){
				xpos += xSpeed;
			   	f->setSpritePos(xpos, ypos);
			   	this_thread::sleep_for(chrono::milliseconds(10));
			}

			// Approach
			f->setCurrentPhase("Approach");
			xSpeed = getRandomSpeed(f->getCurrentPhase()) / 100.0f;
			cout << f->getCurrentPhase() << " " << f->getID() << endl;
			while(movementClockA.getElapsedTime().asSeconds() >= 0.5 && movementClockA.getElapsedTime().asSeconds() < 1.5){
				xpos += xSpeed;
			   	f->setSpritePos(xpos, ypos);
			   	this_thread::sleep_for(chrono::milliseconds(10));
			}
		   ...
		   // Taxi
		   	f->setCurrentPhase("Taxi");
			xSpeed = getRandomSpeed(f->getCurrentPhase()) / 100.0f;
			cout << f->getCurrentPhase() << " " << f->getID() << endl;
			movementClockA.restart();
			while(flightAngle < 180){
				if(movementClockA.getElapsedTime().asSeconds() > 1.0f){
					flightAngle += 0.95f;
					f->setAngle(flightAngle);
					ypos += ySpeed;
				}
				xpos += xSpeed;					
				f->setSpritePos(xpos, ypos);
				f->setSpriteRotation(flightAngle);
				this_thread::sleep_for(chrono::milliseconds(10));
			}
			runway->setOccupied(false);
			pthread_mutex_unlock(mutex);
			cout << "Landing phases complete\n";
		}

		// RUNWAY B case (Departure flights)
		else if (runway->getRunwayID() == "RWY-B") {
			float xSpeed = xSpeed = getRandomSpeed(f->getCurrentPhase()) / 100.0f;
			float ySpeed = 2.0f;
			int xpos = f->getSprite().getPosition().x;
			int ypos = f->getSprite().getPosition().y;
			float flightAngle = 0;
			...
			f->setCurrentPhase("Taxi");
			cout << f->getCurrentPhase() << " " << f->getID() << endl;
			// movementClockB.restart();
			while(movementClockB.getElapsedTime().asSeconds() < 1.75f){
				if(movementClockB.getElapsedTime().asSeconds() > 0.8f){
					if(flightAngle > -90){
						flightAngle -= 1.02f;
					}
					xpos -= xSpeed;
				}
				if(movementClockB.getElapsedTime().asSeconds() < 0.1f){ 
					ypos -= ySpeed;
				}
				f->setAngle(flightAngle);
				f->setSpritePos(xpos, ypos);
			...
			movementClockB.restart();
			Clock takeoffClock;
			f->setCurrentPhase("Takeoff Roll");
			xSpeed = getRandomSpeed(f->getCurrentPhase()) / 100.0f;
			cout << f->getCurrentPhase() << " " << f->getID() << endl;
			while (takeoffClock.getElapsedTime().asSeconds() < 5.0f) {
				xpos -= xSpeed;
				f->setSpritePos(xpos, ypos);
				f->setAngle(flightAngle); // Set the angle
				f->setSpriteRotation(flightAngle);
				this_thread::sleep_for(chrono::milliseconds(10));
			}
			...
			// Cruise
			Clock cruiseClk;
			f->setCurrentPhase("Cruise");
			xSpeed = getRandomSpeed(f->getCurrentPhase()) / 100.0f;
			cout << f->getCurrentPhase() << " " << f->getID() << endl;
			while(cruiseClk.getElapsedTime().asSeconds() < 1.0){
				xpos -= xSpeed;
				f->setSpritePos(xpos, ypos);
				this_thread::sleep_for(chrono::milliseconds(10));
			}
			...
			int speed = getRandomSpeed(f->getCurrentPhase());
			float xSpeed = f->getSpeed() / 100.0f;
			float xtaxi = f->getSpeed() / 10.0f;
			float ySpeed = f->getSpeed() / 150.0f;
			int xpos = f->getSprite().getPosition().x;
			int ypos = f->getSprite().getPosition().y;
			float flightAngle = f->getAngle(); // Get the angle
			int xLim = 1040;

			cout << f->getCurrentPhase() << " " << f->getID() << endl;
			speed = getRandomSpeed(f->getCurrentPhase());
			xSpeed = speed / 150.0f;

			// If departure plane
			if(f->getCurrentPhase() == "At Gate"){
				// Takeoff case on runway C
				movementClockC.restart();
				while(movementClockC.getElapsedTime().asSeconds() < 2.0f){
					flightAngle -= 0.5f;
					f->setSpriteRotation(flightAngle);
					xpos -= 1;
					f->setSpritePos(xpos, ypos);
			...
			// If arrival plane
			else if (f->getCurrentPhase() == "Holding"){
				
				flightAngle = 90;
				f->setSpriteRotation(flightAngle);
				xSpeed = getRandomSpeed(f->getCurrentPhase()) / 100.0f;
				cout << red << f->getCurrentPhase() << " " << f->getID() << default_text << endl;
				while(xpos < -200){
					xpos += xSpeed;
					f->setSpritePos(xpos, ypos);
					this_thread::sleep_for(chrono::milliseconds(10));
...
class ATC {
	...
	public:
		ATC()
		: rwyA("RWY-A", false), rwyB("RWY-B", false), rwyC("RWY-C", false), window(VideoMode(1520, 950), "ATC Simulator") {
			texture.loadFromFile("atc2.png");
		...
		void initializeSystem() {
			...
			vector<Flight*> piaFlights = {
				new Flight("PK-123", "Passenger", "North", (rand() % 300) + 200, "Holding", &rwyA, aircraftsPia[0]),
				new Flight("PK-713", "Passenger", "North", (rand() % 300) + 200, "Holding", &rwyA, aircraftsPia[1]),
				new Flight("PK-234", "Passenger", "North", (rand() % 300) + 200, "Holding", &rwyA, aircraftsPia[2]),
				new Flight("PK-456", "Passenger", "East", rand() % 10, "At Gate", &rwyB, aircraftsPia[3])
			};
			...
			vector<Flight*> fedexFlights = {
				new Flight("FX-789", "Cargo", "South", (rand() % 300) + 200, "Holding", &rwyC, aircraftsFedex[0]),
			...
			vector<Flight*> airBlueFlights = {
				new Flight("AB-501", "Passenger", "North", (rand() % 300) + 200, "Holding", &rwyA, aircraftsAirBlue[0]),
				new Flight("AB-502", "Passenger", "South", (rand() % 300) + 200, "Holding", &rwyA, aircraftsAirBlue[1]),
			...
			vector<Flight*> blueDartFlights = {
				new Flight("BD-301", "Cargo", "South", (rand() % 300) + 200, "Holding", &rwyC, aircraftsBlueDart[0]),
			...
			vector<Flight*> agaFlights = {
				new Flight("AGA-201", "Medical", "North", (rand() % 300) + 200, "Holding", &rwyA, aircraftsAGA[0])
			};
		...
		void initialize2() {
			...
			vector<Flight*> piaFlights = {
				...
				new Flight("PK-7295", "Passenger", "East", 0, "At Gate", &rwyB, aircraftsPia[2]),
				new Flight("PK-456", "Passenger", "East", 0, "At Gate", &rwyB, aircraftsPia[3]),
				new Flight("PK-234", "Passenger", "North", 400, "Holding", &rwyA, aircraftsPia[4]),
			};
        ...
        // Perform initial setup
			initialize2();  // Or initializeSystem(), as needed
			simulate3();    // Start flight threads
			...
			while (window.isOpen()) {
				...
				cout << "Mouse Position: (" << mousePos.x << ", " << mousePos.y << ")\r";
				cout.flush();
				window.clear();
				window.draw(sprite);

				// Iterate through allFlights and draw them
				for (Flight* flight : allFlights) {
					if (flight) {
						window.draw(flight->getSprite());
					}
					else {
				...
				window.display();
				//this_thread::sleep_for(chrono::milliseconds(10)); // Add a small delay

			}
			// Clean up threads (optional, if needed)
			for (pthread_t thread : flightThreads) {
		...
		void simulate3(){
			...
			// 		pthread_t tid;
			// 		pthread_create(&tid, NULL, handleFlights2, (void*)f);
